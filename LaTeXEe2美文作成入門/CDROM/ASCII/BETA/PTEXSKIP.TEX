...
\ProvidesFile{ptexskip.tex}[1997/07/12 v0.3 about pTeX skips]
\usepackage{plext}
...
\section*{はじめに}
p\TeX{}は、以下の条件をすべてクリアした箇所に四分アキを挿入する。
...
テキストモードのときには文字が並んでいるだけなので、上記の制限があっても、
思いどおりの結果を得ることができるが、数式では、数式内部の文字の位置を
調整するためにシフトされたり、\cs{vbox}に入れられたりするために、
思っていたように四分アキが入らないという結果となることがある。

そこで、p\TeX~2.1.5では、テキスト内数式の前後にも四分アキ（あるいは
和文間スペース）が入るチャンスを広げるように拡張した。
この文書では、その拡張に際しての仕様の変更について簡単に説明をしている。

\section{p\TeX~2.1.4までの数式の前後}
p\TeX~2.1.4で\footnote{より正確に言えば、p\TeX~2.1.5 $\beta$~6まで}、
テキスト内数式の前や後に四分アキが入らないときの、
主な原因は以下の３つである。
...
すなわち、数式の最後が和文であれば漢字間スペースを挿入し、
欧文であれば四分アキを挿入する。シフトされた\cs{hbox}や\cs{vbox}など
のときは、決まっていない（挿入しないことになっている）が、このときは、
前側のスペースと合わせる意味で四分アキを挿入する。
これで$x^2$や$\frac{dx}{dy}$や$\sqrt{3}$などの後ろにもスペースが入る。

なお、数式内部の最後の文字の\cs{xspcode}も無視するようにし、
...
四分アキは入らない。つまり“\verb|（$y=a+b$）|”は“（$y=a+b$）”となり
“（\kern\xkanjiskip$y=a+b$\kern\xkanjiskip）”とはならないことに注意する。
...
四分アキと決まっているので、結果もわかりやすいと思う。
そこで$\beta8$では、この節の冒頭で述べた、
\begin{center}
単純にテキスト数式の前後に四分アキを入れる
\end{center}
という仕様で実装している。
